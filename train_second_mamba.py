...
import time
from time_calculate import calculate_time_taken  # Import the function
import glob
...
def main(config_path):
    ...
    # write logs
    file_handler = logging.FileHandler(osp.join(log_dir, 'train_mamba.log'))
    ...
    # Retrieve `max_epoch_files` from config
    max_epoch_files = config.get('max_epoch_files', 10)
    ...
    resume_checkpoint_path = osp.join(config['log_dir'], resume_checkpoint_filename) if resume_checkpoint_filename else ''  # Construct the full path

    if resume_checkpoint_path and osp.exists(resume_checkpoint_path):
        # If checkpoint exists, resume from it
        print(f"Resuming training from checkpoint: {resume_checkpoint_path}")
        model, optimizer, start_epoch, iters = load_checkpoint(model, optimizer, resume_checkpoint_path,
                                        load_only_params=config.get('load_only_params', False))  # Load full checkpoint
        start_epoch = start_epoch + 1
    elif config.get('pretrained_model', ''):
        # If no checkpoint but pretrained model is specified, load it
        print(f"Loading pretrained model from: {config['pretrained_model']}")
        model, optimizer, start_epoch, iters = load_checkpoint(model, optimizer, config['pretrained_model'],
                                        load_only_params=config.get('load_only_params', True))  # Load only parameters
    else:
        # If no checkpoint or pretrained model, start from scratch
        print("No checkpoint or pretrained model found. Starting training from scratch.")
        start_epoch = 0
        iters = 0
    ...
    for epoch in tqdm(range(start_epoch, epochs), desc="Epochs"):
        ...
        for i, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{epochs} - Training")):
            ...
            for _s2s_pred, _text_input, _text_length in zip(d, d_gt, input_lengths):
                ...
                # Clamp _s2s_pred to a valid range to avoid NaN/Inf issues
                _s2s_pred = torch.clamp(_s2s_pred, min=-1e6, max=1e6)
                ...
                # Apply sigmoid and check for NaN/Inf
                _dur_pred = torch.sigmoid(_s2s_pred).sum(axis=1)
                if torch.isnan(_dur_pred).any() or torch.isinf(_dur_pred).any():
                    print("NaN or Inf detected in _dur_pred. Skipping this batch.")
                    continue
            ...
            if epoch >= joint_epoch:
                ...
                for p in model.predictor.mamba_ssm.parameters():
                    if p.grad is not None:
                        p.grad *= slmadv_params.scale
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(val_dataloader, desc=f"Epoch {epoch+1}/{epochs} - Validation")):
                ...
                try:
                    ...
                    if gt.dim() == 3:
                        gt = gt.unsqueeze(1)  # Add channel dimension if it's missing
                    ...
                    # Pass the correctly shaped gt to pitch_extractor
                    try:
                        F0_real, _, F0 = model.pitch_extractor(gt)

                    except Exception as e:
                        print(f"Error in pitch_extractor: {e}")
        ...
        if iters_test > 0:
            avg_val_loss = loss_test / iters_test
            ...
            # Check if the current epoch's validation loss is the best seen so far
            if avg_val_loss < best_loss:
                ...
                best_save_path = osp.join(log_dir, f'best_loss_epoch.pth')
                torch.save(best_state, best_save_path)
                print(f"New best model saved with validation loss: {best_loss:.5f} at epoch {epoch}")
            ...
            logger.info('No batches processed during validation.')
        ...
        if epoch < joint_epoch:
            ...
            with torch.no_grad():
                ...
                for bib in range(len(d_en)):
                    ...
                    # print(f"Shape of duration after duration_proj: {duration.shape}")  # Verify duration's shape
                    ...
                    # Normalize the audio tensor to stay within [-1, 1]
                    out = out / out.abs().max()
        ...
        if epoch % saving_epoch == 0:
            ...
            save_path = osp.join(log_dir, f'epoch_2nd_{epoch+1:05d}.pth')
            torch.save(state, save_path)

            # Check if the number of epoch files exceeds `max_epoch_files`
            epoch_files = sorted(glob.glob(osp.join(log_dir, 'epoch_2nd_*.pth')))
            if len(epoch_files) > max_epoch_files:
                # Remove the oldest file(s) to keep only the latest `max_epoch_files` files
                file_to_delete = epoch_files[0]
                os.remove(file_to_delete)
                print(f"Deleted old checkpoint file: {file_to_delete}")
...
if __name__ == "__main__":
    ...
    try:
        main()  # Run the main training function
    finally:
        end_time = time.time()  # Record the end time

        # Call the imported time calculation function
        days, hours, minutes, seconds = calculate_time_taken(start_time, end_time)
        
        # Log the result in both the console and the log file
        logger.info(f"Total time taken for train_finetune: {days} days, {hours} hours, {minutes} minutes, {seconds:.2f} seconds" + '\n\n')
        print(f"Total time taken for train_finetune: {days} days, {hours} hours, {minutes} minutes, {seconds:.2f} seconds")
